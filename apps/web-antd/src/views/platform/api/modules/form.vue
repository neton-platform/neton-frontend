...
const rules: Record<string, Rule[]> = {
  ...
  httpMethod: [
    { required: true, message: 'HTTP 方法不能为空', trigger: 'blur' },
  ],
  status: [{ required: true, message: '状态不能为空', trigger: 'blur' }],
  isPublic: [{ required: true, message: '是否公开不能为空', trigger: 'blur' }],
  rateLimitPerMin: [
    { required: true, message: '每分钟限流不能为空', trigger: 'blur' },
  ],
  chargeType: [
    { required: true, message: '计费类型不能为空', trigger: 'change' },
  ],
  defaultPrice: [
    { required: true, message: '默认单价（分）不能为空', trigger: 'blur' },
  ],
};
    ...
    >
      ...
      <Form.Item label="HTTP 方法" name="httpMethod">
        <Select v-model:value="formData.httpMethod">
          <Option
            v-for="dict in getDictOptions(DICT_TYPE.PLATFORM_REQUEST_METHOD)"
            :key="dict.value"
            :value="dict.value"
          >
            {{ dict.label }}
      ...
      <Form.Item label="计费类型" name="chargeType">
        <Select
          v-model:value="formData.chargeType"
          placeholder="请选择计费类型"
        >
          <Select.Option
            v-for="dict in getDictOptions(
              DICT_TYPE.PLATFORM_CHARGE_TYPE,
              'number',
            )"
            :key="dict.value"
            :value="dict.value"
      ...
      <Form.Item label="默认单价（分）" name="defaultPrice">
        <Input
          v-model:value="formData.defaultPrice"
