...
export namespace LogApi {
  ...
  export interface Log {
    ...
    apiId?: number | string; // API ID（关联 platform_api.id）
    ...
    responseStatus?: number | string; // HTTP 状态码
    responseBody?: string; // 响应内容（截断，保留前 10KB）
    durationMs?: number; // 耗时（毫秒）
    success?: boolean | string; // 是否成功
    errorCode?: string; // 错误码
    errorMsg?: string; // 错误信息
    chargePrice?: number; // 本次计费金额（分）
    chargeStatus?: number | string; // 扣费状态：1=成功 2=失败（余额不足）
...
export function getLogPage(params: PageParam) {
  return requestClient.get<PageResult<LogApi.Log>>('/platform/log/page', {
    params,
  });
}
