...
const resetListenersList = () => {
  ...
  otherExtensionList.value =
    ...
    ) ?? []; // 保留非监听器类型的扩展属性，避免移除监听器时清空其他配置（如审批人等）。相关案例：https://gitee.com/netoncode/neton-ui-admin-vue3/issues/ICMSYC
      ...
      <ElForm :model="listenerForm" ref="listenerFormRef" label-width="90px">
        ...
        >
          <ElSelect v-model="listenerForm.event">
            <ElOption value="start" label="start" />
            <ElOption value="end" label="end" />
          </ElSelect>
        </ElFormItem>
        <ElFormItem
          label="监听器类型"
