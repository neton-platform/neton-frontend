...
<template>
  ...
  >
    <ElCollapse v-model="activeTab" v-if="isReady">
      <ElCollapseItem name="base" title="常规">
        <template #icon>
      ...
      <ElCollapseItem name="form" title="表单" v-if="formVisible">
        <template #icon>
      ...
      <ElCollapseItem
        name="task"
        :title="getTaskCollapseItemName(elementType)"
      ...
      <ElCollapseItem name="listeners" title="执行监听器">
        <template #icon>
      ...
      <ElCollapseItem name="extensions" title="扩展属性">
        <template #icon>
          <IconifyIcon icon="ep:circle-plus-filled" />
        </template>
        <ElementProperties :id="elementId" :type="elementType" />
      </ElCollapseItem>
      <ElCollapseItem name="other" title="其他">
        <template #icon>
          <IconifyIcon icon="ep:promotion" />
        </template>
        <ElementOtherConfig :id="elementId" />
      </ElCollapseItem>
      <ElCollapseItem name="customConfig" title="自定义配置">
        <template #icon>
      ...
      >
        ...
        <!-- 相关 issue：https://gitee.com/netoncode/neton-ui-admin-vue3/issues/ICNRW2 -->
        <TimeEventConfig
          :business-object="elementBusinessObject"
          :key="elementId"
        />
